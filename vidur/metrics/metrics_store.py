...
class MetricsStore:
    ...
    @if_write_metrics
    def plot(self, final_time: Optional[float] = None) -> None:
        dir_plot_path = f"{self._config.output_dir}/plots"
        os.makedirs(dir_plot_path, exist_ok=True)

        if final_time is not None:
            self._finalize_request_metrics(final_time)
    ...
    def _on_request_end(self, time: float, request: Request) -> None:
        ...
        self._request_metrics_histogram[RequestMetricsHistogram.REQUEST_COMPLETED].put(
            request.id, 1
        )
        self._completed_request_ids.add(request.id)

    def _finalize_request_metrics(self, final_time: float) -> None:
        if not self._config.store_request_metrics:
            return
        if not self._requests_by_id:
            return

        e2e_series = self._request_metrics_time_distributions[
            RequestMetricsTimeDistributions.REQUEST_E2E_TIME
        ]
        e2e_norm_series = self._request_metrics_time_distributions[
            RequestMetricsTimeDistributions.REQUEST_E2E_TIME_NORMALIZED
        ]
        completion_series = self._request_metrics_histogram[
            RequestMetricsHistogram.REQUEST_COMPLETED
        ]

        for request_id, request in self._requests_by_id.items():
            if request_id in self._completed_request_ids:
                continue
            latency = max(0.0, float(final_time) - float(request.arrived_at))
            e2e_series.put(request_id, latency)
            denom = float(request.num_decode_tokens)
            if denom <= 0:
                denom = 1.0
            e2e_norm_series.put(request_id, latency / denom)
            completion_series.put(request_id, 0)
