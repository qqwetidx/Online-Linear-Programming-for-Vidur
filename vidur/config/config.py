...
class TraceRequestLengthGeneratorConfig(BaseRequestLengthGeneratorConfig):
    ...
    prefill_column: str = field(
        default="num_prefill_tokens",
        metadata={
            "help": "Prefill column name for the trace request length generator."
        },
    )
    decode_column: str = field(
        default="num_decode_tokens",
        metadata={
            "help": "Decode column name for the trace request length generator."
        },
    )
    shuffle: bool = field(
        default=True,
        metadata={"help": "Shuffle trace request lengths before use."},
    )
...
@dataclass
class OnlineLpGlobalSchedulerConfig(BaseGlobalSchedulerConfig):
    policy: str = field(
        default="dual_ahd",
        metadata={"help": "Online LP policy: dual_ahd."},
    )
    horizon: int = field(
        default=2048,
        metadata={"help": "Discrete horizon (steps) for Online LP scheduling."},
    )
    time_step_seconds: float = field(
        default=1.0,
        metadata={"help": "Seconds per Online LP step for time discretization."},
    )
    total_jobs: int = field(
        default=0,
        metadata={"help": "Total jobs estimate for Online LP; 0 uses auto estimate."},
    )
    reward_normalizer: float = field(
        default=1.0,
        metadata={"help": "Reward normalizer for Online LP dual updates."},
    )
    alpha: float = field(
        default=1.0,
        metadata={"help": "Reward weight for throughput term."},
    )
    beta: float = field(
        default=0.0,
        metadata={"help": "Reward weight for completion term."},
    )
    gamma: float = field(
        default=0.0,
        metadata={"help": "Reward weight for first-token term."},
    )
    sigma: float = field(
        default=0.0,
        metadata={"help": "Reward constant term."},
    )
    goodput_window: int = field(
        default=0,
        metadata={"help": "Goodput window (steps) for bonus reward."},
    )
    goodput_weight: float = field(
        default=0.0,
        metadata={"help": "Goodput bonus weight."},
    )
    grad_steps: int = field(
        default=5,
        metadata={"help": "Projected gradient steps for dual updates."},
    )
    step_size: float = field(
        default=0.01,
        metadata={"help": "Step size for dual updates."},
    )
    batch_size: int = field(
        default=64,
        metadata={"help": "Batch size for stochastic dual updates."},
    )
    mem_scale_factor: float = field(
        default=1.0,
        metadata={"help": "Scale factor for memory dual prices."},
    )
    backlog_gain: float = field(
        default=1.0,
        metadata={"help": "Backlog gain for dual updates."},
    )
    backlog_slack: float = field(
        default=0.0,
        metadata={"help": "Backlog slack for dual updates."},
    )
    length_penalty: float = field(
        default=0.0,
        metadata={"help": "Length penalty for Online LP candidate scoring."},
    )
    max_waiting: int = field(
        default=0,
        metadata={
            "help": "Max waiting requests to consider per scheduling step; 0 means no limit."
        },
    )

    @staticmethod
    def get_type():
        return GlobalSchedulerType.ONLINE_LP
